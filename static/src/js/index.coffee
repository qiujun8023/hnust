...
hnust.config ($animateProvider, $httpProvider, $routeProvider) ->
    ...
    $routeProvider
        ...
        .when '/agreement',
            title: '用户使用协议'
            templateUrl: 'index-agreement.html'
        .when '/user',
            title: '用户中心'
            module: 'user'
            controller: 'user'
        ...
        .when '/api',
            title: 'API文档'
            controller: 'api'
            templateUrl: 'index-api.html'
        .when '/schedule',
            title: '实时课表'
            controller: 'schedule'
            templateUrl: 'index-schedule.html'
        .when '/score',
            title: '成绩查询'
            controller: 'score'
            templateUrl: 'index-score.html'
        .when '/scoreAll',
            title: '全班成绩'
            controller: 'scoreAll'
            templateUrl: 'index-scoreAll.html'
        .when '/exam',
            title: '考试安排'
            controller: 'exam'
            templateUrl: 'index-exam.html'
        .when '/credit',
            title: '学分绩点'
            controller: 'credit'
            templateUrl: 'index-credit.html'
        .when '/classroom',
            title: '空闲教室'
            controller: 'classroom'
            templateUrl: 'index-classroom.html'
        .when '/elective',
            title: '选课平台'
            controller: 'elective'
            templateUrl: 'index-elective.html'
        .when '/judge',
            title: '教学评价'
            controller: 'judge'
            templateUrl: 'index-judge.html'
        .when '/rank',
            title: '成绩排名'
            controller: 'rank'
            templateUrl: 'index-rank.html'
        .when '/book',
            title: '图书借阅'
            controller: 'book'
        ...
        .when '/tuition',
            title: '学年学费'
            controller: 'tuition'
            templateUrl: 'index-tuition.html'
        .when '/card',
            title: '一卡通'
            controller: 'card'
            templateUrl: 'index-card.html'
        .when '/failRate',
            title: '挂科率'
            module: 'data'
            controller: 'failRate'
            templateUrl: 'index-failRate.html'
        .otherwise
            redirectTo: '/schedule'
...
#用户中心
hnust.controller 'user', ($scope, $rootScope, md5, request) ->
    #监视有无用户信息变化
    $scope.$watch ->
        $rootScope.user
    , ->
...
#登录
hnust.controller 'login', ($scope, $rootScope, $cookies, $location, md5, request) ->
...
#成绩
hnust.controller 'api', ($scope) ->
    $scope.baseUrl  = window.location.protocol + '//'
    $scope.baseUrl += window.location.host

#成绩
hnust.controller 'score', ($scope, request) ->
    $('.ui.form').form()
    $scope.error   = ''
    $scope.loading = true
...
#全班成绩
hnust.controller 'scoreAll', ($scope, $location, request) ->
...
#课表
hnust.controller 'schedule', ($scope, $timeout, request, utils) ->
    $scope.weeks = []
    for i in [0..20]
...
#考试
hnust.controller 'exam', ($scope, request) ->
    $scope.error   = ''
    $scope.loading = true
    request.query {}, (error, info, data) ->
        $scope.loading = false
...
#学分绩点
hnust.controller 'credit', ($scope, request) ->
    $scope.error   = ''
    $scope.loading = true
    request.query
        timeout: 30000
...
#空闲教室
hnust.controller 'classroom', ($scope, $rootScope, request, utils) ->
    #教学楼代码
    $scope.builds = [
        ['103', '第一教学楼']
        ['102', '第二教学楼']
...
#选课平台
hnust.controller 'elective', ($scope, request) ->
    $('.tabular .item').tab()
...
#教学评价
hnust.controller 'judge', ($scope, request) ->
    #获取评教列表
    $scope.list = ->
        $scope.loading = true
        request.query {}, (error, info, data) ->
    ...
    $scope.auto =
        ...
        radio  : ->
            ...
            if Math.max.apply(Math, radio) is Math.min.apply(Math, radio)
                self.radio()
            else
                radio
        action : ->
            self = this
            self.data = $scope.data.filter (item) ->
...
#成绩排名
hnust.controller 'rank', ($scope, $timeout, $filter, request) ->
    $scope.error   = ''
    $scope.loading = true
    request.query {}, (error, info, data) ->
        $scope.loading = false
...
#图书借阅
hnust.controller 'book', ($scope, $timeout, request) ->
    $('.tabular .item').tab()
    #回车键Submit
    $('.ui.form').form {},
        onSuccess: ->
    ...
    $scope.search = (key, page) ->
        ...
        #关键词为空或正在加载中
        if $scope.list.loading or !$scope.key.length then return
        #判断页码
        page = if page and page > 0 then parseInt(page) else 1
        if page is 1 then $scope.list.data = []
        #加载图书信息
        $scope.list.loading = true
        request.query
            params:
...
#学费
hnust.controller 'tuition', ($scope, $timeout, request) ->
    $scope.error   = ''
    $scope.loading = true
    request.query {}, (error, info, data) ->
        $scope.loading = false
...
#校园一卡通
hnust.controller 'card', ($scope, $rootScope, $filter, request) ->
    $scope.data =
        per   : 30
        page  : 1
        data  : []
...
#挂科率统计
hnust.controller 'failRate', ($scope, $rootScope, $timeout, $filter, request) ->
    $scope.data =
        per   : 30
        page  : 1
        data  : []
...
#挂科率统计
hnust.controller 'sexRatio', ($scope, $location, request) ->
    #搜索
    $scope.search = (key) ->
        if key then $scope.key = key
        if !$scope.key then return
